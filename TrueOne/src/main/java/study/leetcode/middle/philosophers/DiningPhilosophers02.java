...
public class DiningPhilosophers02 {
    private static final int EAT_TIMES = 8;
    private static final int PHILOSOPHERS_NUMBERS = 5;
    final Semaphore[] semaphores = new Semaphore[PHILOSOPHERS_NUMBERS];
    ...
    public void wantsToEat(int philosopher,
        ...
        Semaphore firstSemaphore = semaphores[philosopher];
        Semaphore secondSemaphore = semaphores[(philosopher + 1) % PHILOSOPHERS_NUMBERS];
        while (true){
            boolean acquireFirst = firstSemaphore.tryAcquire();
            boolean acquireSecond = secondSemaphore.tryAcquire();
            if (acquireFirst && acquireSecond){
                pickLeftFork.run();
                pickRightFork.run();
                eat.run();
                putLeftFork.run();
                ...
                break;
            }else if(acquireFirst){
                firstSemaphore.release();
            }else if(acquireSecond){
                secondSemaphore.release();
            }
        }
    }
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        DiningPhilosophers02 diningPhilosophers = new DiningPhilosophers02();
        for (int i = 0; i < PHILOSOPHERS_NUMBERS; i++) {
            final int index = i;
            new Thread(() -> {
                try {
